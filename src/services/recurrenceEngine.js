...
export const generateTaskInstances = (template, startDate, endDate) => {
  const occurrences = generateOccurrences(template.recurrence, startDate, endDate, {
    includeFirst: false, // Don't include the template creation date
  })
  ...
  return occurrences.map((date, index) => ({
    ...
    dueDate: format(computeDueDate(date), 'yyyy-MM-dd'),
    tags: [...(template.tags || [])],
