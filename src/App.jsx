...
const COMPLETION_MARKERS = [/已完成/g, /做完/g, /完成了/g]

const extractCompletion = (text) => {
  if (!text) return { completed: false, stripped: '' }
  let completed = false
  let stripped = text
...
const parseInput = (raw) => {
  ...
  const { completed, stripped: completionStripped } = extractCompletion(normalized)
  const fundResult = parseFund(completionStripped)
  ...
  const title = working || completionStripped
...
function App() {
  ...
  const handleGenerateFromTemplate = (template) => {
    ...
    if (instances.length === 0) {
      const notifyDays = Number(template?.recurrence?.notifyDaysBefore || 0)
      if (notifyDays > 0) {
        alert(`尚未到提醒窗口（DDL前${notifyDays}天）`)
        return
      }
  ...
  const handleAddTask = (input) => {
    ...
    let parsed = typeof input === 'string'
      ? (() => {
    ...
    const completionFromTitle = extractCompletion(parsed.title || '')
    if (completionFromTitle.completed) {
      parsed = {
        ...parsed,
        completed: true,
        title: completionFromTitle.stripped || parsed.title,
      }
    }

    if (typeof input !== 'string' && input?.raw) {
      const completionFromRaw = extractCompletion(input.raw)
      if (completionFromRaw.completed) {
        parsed = {
          ...parsed,
          completed: true,
        }
  ...
  const handleAddChecklistItem = (taskId, item) => {
    setTasks((prev) =>
      prev.map((task) =>
        task.id === taskId ? TaskOperations.addChecklistItem(task, item) : task
      )
    )
  }
  ...
  const handleAddTaskTag = (taskId, tag) => {
    setTasks((prev) =>
      prev.map((task) =>
        task.id === taskId ? TaskOperations.addTag(task, tag) : task
      )
    )
  }

  const handleUpdateTaskTag = (taskId, previousTag, nextTag) => {
    setTasks((prev) =>
      prev.map((task) =>
        task.id === taskId ? TaskOperations.updateTag(task, previousTag, nextTag) : task
      )
    )
  }

  const handleUpdateTaskNote = (taskId, note, noteRefined) => {
    setTasks((prev) =>
      prev.map((task) =>
        task.id === taskId ? TaskOperations.updateNote(task, note, noteRefined) : task
      )
    )
  }
  ...
  const tasksForDate = (date) => {
    ...
    return tasks.filter((task) => {
      if (task.completed) {
        return task.completedDate === dayKey
      }
      return task.dueDate === dayKey && shouldShowTask(task, dayKey)
    })
  }
  ...
  return (
    <div className="min-h-screen bg-[var(--surface-0)] text-[var(--text-900)] overflow-hidden">
      <div className="flex min-h-screen">
        <aside className="w-72 bg-[var(--navy-950)] text-slate-100 flex flex-col px-5 py-6 min-h-screen">
          <div>
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                <p className="text-sm text-slate-300">PE Fund Ops</p>
                  <h1 className="text-lg font-semibold tracking-wide">智能任务中枢</h1>
              </div>
            </div>
